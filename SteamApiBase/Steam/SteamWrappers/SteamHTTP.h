...
class SteamHTTP002 : public ISteamHTTP002
{
public:
    // Initializes a new HTTP request, returning a handle to use in further operations on it.  Requires
    // the method (GET or POST) and the absolute URL for the request.  Only http requests (ie, not https) are
    ...
    bool GetHTTPStreamingResponseBodyData( HTTPRequestHandle hRequest, uint32, uint8 *pBodyDataBuffer, uint32 unBufferSize );
