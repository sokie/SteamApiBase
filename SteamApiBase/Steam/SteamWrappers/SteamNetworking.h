...
class SteamNetworking001 : public ISteamNetworking001
{
public:
	SNetListenSocket_t CreateListenSocket(int nP2PPort, uint32 nIP, uint16 nPort);

	// creates a socket and begin connection to a remote destination
	// can connect via a known steamID (client or game server), or directly to an IP
	// on success will trigger a SocketStatusCallback_t callback
	// on failure or timeout will trigger a SocketStatusCallback_t callback with a failure code in m_eSNetSocketState
	SNetSocket_t CreateP2PConnectionSocket(CSteamID steamIDTarget, int nPort, int nTimeoutSec);
	...
	// returns false if no data is available
	// fills out *phSocket with the socket that data is available on
	bool RetrieveData(SNetListenSocket_t hListenSocket, void *pubDest, uint32 cubDest, uint32 *pcubMsgSize);
...
class SteamNetworking002 : public ISteamNetworking002
{
public:
	...
	// returns information about the specified socket, filling out the contents of the pointers
	bool GetSocketInfo(SNetSocket_t hSocket, CSteamID *pSteamIDRemote, ESNetSocketState *peSocketStatus, uint32 *punIPRemote, uint16 *punPortRemote);
